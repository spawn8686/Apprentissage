 ...
 *
 */
public class P0525 extends AdvancedRobot {
	...
	public void run() {
		...
		while (true) {
			...
			if (Math.abs(getGunTurnRemaining()) < 1 && getGunHeat() == 0) {
				waves.add(new Wave(this, enemyLocation.getAngle(), FIRE_POWER));
				setFire(FIRE_POWER);
			}
			execute();
		}
	...
	public void onScannedRobot(ScannedRobotEvent event) {
		...
		// Est-ce que une onde a atteint l'ennemi
		Wave wave;
		for (int i = 0; i < waves.size(); i++) {
			wave = waves.get(i);
			if (wave.reaches(enemyLocation, getTime())) {
	...
	public void onPaint(Graphics2D g) {
		...
		for (int i = 0; i < waves.size(); i++) {
			...
			g.draw(waves.get(i).getShape(getTime()));
		}
	}
